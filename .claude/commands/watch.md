...
2. For each watched trip, dispatch on `trip.source` (see `.claude/skills/price-watch/SKILL.md` "Re-check procedure" for the authoritative version of this logic):
   ...
   - **Browser-driven** (`trivago-search`, `momondo-search`, `booking-search`): re-run the same search via the Playwright MCP driver per `.claude/skills/trivago-search/SKILL.md`'s driver contract. On a chain-ending condition (bot challenge, consent wall, zero results, unreadable page), fall through to web search. If that also yields nothing, record `available: false` and note the reason in the report text (not the JSON) — report it as a **blocked read**, distinct from a genuine sold-out.
