...
- `<FRAMEWORK_ROOT>/skills/price-watch/SKILL.md` — authoritative definition of the JSON shape and re-check logic; defer to it for any detail not covered here.
...
- `<FRAMEWORK_ROOT>/skills/trivago-search/SKILL.md` — authoritative driver contract (server config, tool-capability mapping, etiquette limits) for re-checking browser-driven trips (`trivago-search`, `momondo-search`, `booking-search`); re-check dispatches to it rather than restating it here.
...
2. Derive the slug per `<FRAMEWORK_ROOT>/skills/price-watch/SKILL.md` — the authoritative source for slug derivation.
3. Read `<FRAMEWORK_ROOT>/skills/price-watch/SKILL.md` for the exact JSON shape expected in `<DATA_ROOT>/watchlist/<slug>.json` and follow it precisely.
...
2. For each watched trip, dispatch on `trip.source` into one of three branches — adapter-backed, browser-driven, or pasted — per `<FRAMEWORK_ROOT>/skills/price-watch/SKILL.md` "Re-check procedure", the sole authority for this logic; do not re-enumerate the triggers or branches here.
