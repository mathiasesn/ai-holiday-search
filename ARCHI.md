...
**Plugin facts, tagged by how each was established.** The end-to-end `/plugin install` of this repo was actually performed, on branch `plugin-distribution` at commit `74ddc64`, on 2026-08-06. Each bullet below is marked **[exercised]** (directly observed in that install) or **[inferred]** (from the Claude Code plugins reference and/or a local symlink-discovery experiment, not exercised by that install):

- **[inferred]** `${CLAUDE_PLUGIN_ROOT}` is interpolated in command and skill Markdown, and is only set when running as an installed plugin — its absence is the clone-mode signal. Not exercised: `/setup`, the only command that interpolates it at runtime, was not run during the install.
...
- **[exercised]** All three PEP 723 adapters, and both markers the commands' fail-loudly fallback keys on (`ARCHI.md`, `.agents/skills/`), exist intact in the installed tree at `.agents/skills/{flights,stays,packages}-search/search.py` — no structural rewrite is needed for plugin mode.
...
- **[exercised]** A directory-source install (`{"source": "directory", ...}` in `known_marketplaces.json`) copies the entire working tree — a real copy, distinct inode, not a symlink — to `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/`, gitignored files included (the personal paths §9 lists, e.g. `profile/`, `trip_tracker.csv`). A GitHub-source install should carry only tracked files and thus be unaffected, but this was not exercised — the GitHub-source route currently fails (`.claude-plugin/` is not on the default branch; see `SETUP.md` step 1), so that claim is an inference, not a confirmed result. Filed as [issue #22](https://github.com/mathiasesn/ai-holiday-search/issues/22); see also §9.

**Still unverified:** a GitHub-source `/plugin install` of this repo (the route currently fails — `.claude-plugin/` is not on the default branch); `${CLAUDE_PLUGIN_ROOT}` actually being interpolated at runtime (needs `/setup` to run); `/setup` actually writing `~/.ai-holiday-search` and its `.gitignore` in plugin mode; MCP server reachability under `/scrape` in plugin mode; cross-project profile reuse.
