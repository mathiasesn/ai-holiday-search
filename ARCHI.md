...
- **Every runtime read target in every `commands/*.md` file must be rooted.** An agent executing a command has no doc-relative base — its cwd is the user's project, not the `commands/` directory — so any backticked path-like span or Markdown-link target that names something the command actually reads or writes at execution time must start with `<FRAMEWORK_ROOT>` or `<DATA_ROOT>`; a bare `skills/…`, a `../skills/…`, or a `[text](../skills/x.md)` link fails the lint. `commands/reset.md`'s protect-list is one instance of this rule: every bullet's backticked path-like span or link target must be rooted, so `../tools/`, a bare `tools/`, or a `[text](../README.md)` link on a protect-list bullet fails the lint. The per-mode "What each mode deletes" bullets carry the same property under a narrower rule — no `../` targets — because those bullets legitimately name bare files in prose (`01-traveler-profile.md`), which the path-like rule would false-positive on. The one exemption that survives is human doc-navigation: links to `../README.md`, `../SETUP.md`, `../ARCHI.md`, `../AGENTS.md` stay relative, because they resolve correctly inside the framework root in both modes and are read by a human clicking through docs, not executed by an agent.
