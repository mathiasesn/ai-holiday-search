...
**Verified plugin facts.** The first four bullets are documentation-derived (Claude Code plugins reference, plus a local symlink-discovery experiment). The end-to-end `/plugin install` of this repo was then actually performed, on branch `plugin-distribution` at commit `74ddc64`, on 2026-08-06, and empirically confirmed the last four bullets:

- `${CLAUDE_PLUGIN_ROOT}` is interpolated in command and skill Markdown, and is only set when running as an installed plugin — its absence is the clone-mode signal.
- An installed plugin's default discovery covers top-level `commands/`, `skills/`, `agents/`, and a root `.mcp.json` automatically — no separate plugin-mode MCP declaration exists or is needed; the same `.mcp.json` (two Playwright servers, §7) serves both modes.
- Claude Code follows the `.claude/commands` and `.claude/skills` symlinks in a clone checkout, which is the entire mechanism that keeps clone mode zero-configuration after the framework content moved to top-level `commands/`/`skills/`.
- `${CLAUDE_PLUGIN_DATA}` (resolves to `~/.claude/plugins/data/<plugin-id>/`) exists as a plugin-managed alternative storage location, but was deliberately **not** used for `DATA_ROOT`. `~/.ai-holiday-search` was chosen instead for user discoverability (a plain, predictable home-directory path a traveler can find, back up, or inspect without knowing Claude Code's internal plugin-data layout) and for cross-project reuse (the same profile applies regardless of which project directory the plugin happens to be invoked from).
- Commands register namespaced, as `/ai-holiday-search:setup` etc., not bare `/setup`; fuzzy matching resolves the bare form. Observed on one Claude Code version on one machine.
- All three PEP 723 adapters, and both markers the commands' fail-loudly fallback keys on (`ARCHI.md`, `.agents/skills/`), exist intact in the installed tree at `.agents/skills/{flights,stays,packages}-search/search.py` — no structural rewrite is needed for plugin mode.
- All five commands register exactly once, and every skill observed in the listing likewise — the tracked `.claude/commands`/`.claude/skills` symlinks are present in the installed tree but do not cause duplicate registration. All six skill directories ship in the installed tree; the listing was only spot-checked, so "no duplicates" is confirmed for the commands and for the skills observed, not exhaustively for all six.
- A directory-source install (`{"source": "directory", ...}` in `known_marketplaces.json`) copies the entire working tree — a real copy, distinct inode, not a symlink — to `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/`, **gitignored files included**: `profile/`, `watchlist/`, `trip_scraper/`, `documents/`, `itineraries/`, `trip_tracker.csv`, `.claude/settings.local.json`, `.playwright-mcp/`. A GitHub-source install is unaffected — it carries only tracked files. Filed as [issue #22](https://github.com/mathiasesn/ai-holiday-search/issues/22); see also §9.

**Still unverified:** `/setup` actually writing `~/.ai-holiday-search` and its `.gitignore` in plugin mode; MCP server reachability under `/scrape` in plugin mode; cross-project profile reuse.
